...
--Потім виділені атрибути групуються по суті, а згодом створюється таблиці. 
--Такий підхід найбільше підходить для проектування невеликих баз даних з невеликою кількістю атрибутів.

--При низхідному підході, спочатку виявляються сутності, а потім їх атрибути та зв'язки між ними

--Атомізація атрибутів
--При визначенні атрибутів відбувається поділ складних комплексних елементів на більш прості. 
...
--У той самий час можливість поділу одного елемента на піделементи не завжди може бути затребуваною. 
...
--Ключі є способом ідентифікації рядків у таблиці. За допомогою ключів ми також можемо зв'язувати рядки між різними таблицями відношеннями.

--Суперключ
--Superkey (суперключ) - комбінація атрибутів (стовпців), які унікально ідентифікують кожен рядок таблиці. 
--Це можуть бути і всі стовпці, і кілька, і один. При цьому поля, які містять значення цих атрибутів, не повинні повторюватися.
...
--Якщо в таблиці можна виділити потенційний ключ, то його можна використовувати в якості первинного ключа.
...
--Зовнішній ключ необов'язково має відповідати первинному ключу з головної таблиці, але повинен вказувати на стовпець з унікальним значенням. 
...
--Цей тип зв'язків зустрічається не часто. І тут об'єкту однієї сутності можна зіставити лише один об'єкт 
--іншої сутності (один рядок з дочірньої таблиці може посилатися на один рядок з батьківської таблиці). 
...
--У свою чергу один рядок з таблиці В може бути пов'язаний з безліччю рядків з таблиці А. 
...
--При зміні первинних і зовнішніх ключів слід дотримуватися такого принципу, як посилальна цілісність даних (referential integrity). 
--Її основна ідея у тому, щоб дві таблиці у базі даних, які зберігають пов'язані дані, підтримували їх узгодженість. 
...
--У цьому випадку стовпець зовнішнього ключа повинен допускати значення NULL.

--Аномалія вставки
--Для вирішення аномалії вставки при додаванні до залежної таблиці даних у стовпець, який представляє зовнішній ключ, 
...
--Аномалії оновлення
--Для вирішення проблеми аномалії оновлення застосовується нормалізація, яка буде розглянута далі.
...
--Нормалізація усуває надмірність даних (data redundancy) і цим допомагає уникнути порушення цілісності даних при її зміні, 
--тобто уникнути аномалій оновлення (update anomaly).
...
--Існує 7 нормальних форм. Кожна нормальна форма (за винятком першої) має вимогу, 
...
--Перша нормальна форма (1NF) передбачає, що дані, що зберігаються на перетині рядків і стовпців повинні представляти скалярне значення (повинні бути атомарними), 
--а таблиці не повинні містити рядків, що повторюються (має бути основний ключ (Primary Key) який унікально ідентифікує рядок).

--Друга нормальна форма (2NF) передбачає, що дані, що зберігаються в таблицях із композитним ключем, не залежали лише від частини ключа. 
--Дані, що повторно з'являються в декількох рядках, виносяться в окремі таблиці.

--Третя нормальна форма (3NF) передбачає, що дані в таблиці залежать винятково від основного ключа.
--Будь-яке поле, що залежить від основного ключа та від будь-якого іншого поля, має виноситись в окрему таблицю.

--Нормальна форма Бойса-Кодда (BCNF) є трохи суворішою версією третьої нормальної форми. І визначає, що відношення знаходиться у цій формі
--тоді і лише тоді, коли детермінант кожної функціональної залежності є потенційним ключем. Якщо це правило не виконується, то, 
--щоб привести вказане відношення до НФБК, його слід розділити на два відношення шляхом двох операцій проєкції на кожну функціональну залежність, детермінант якої не є потенційним ключем:
...
--П'ята нормальна форма (5NF) поділяє таблиці на більш малі таблиці для усунення надмірності даних. Розбиття йде доти, 
...
--але без обмежень на рівні таблиць чи стовпців. Ця форма, зазвичай, не застосовна лише на рівні СУБД, зокрема й у SQL Server.
...
--Причому атрибути А можуть складатися з одного або декількох атрибутів. 
...
--При цьому для одного значення атрибуту В можуть існувати кілька різних значень атрибуту А. Атрибут А в цій залежності ще називається детермінантом .
...
--Існують два підходи для переходу ненормалізованої таблиці до першої нормальної форми. Перший спосіб називається вирівнюванням або flattaning. 
--Він передбачає декомпозицію рядка з групами даних, що повторюються, при якій, для кожної групи, що повторюється, створюється свій рядок. 
...
--У другій нормальній формі кожен стовпець у таблиці, який не є ключем, повинен залежати від ключа .
...
--Якщо атрибут залежить тільки від будь-якої підмножини з атрибуту А, наприклад, тільки від А1, то має місце часткова функціональна залежність.
...
--Якщо у таблиці не складовий первинний ключ, то цьому випадку вважається, 
--що й інші атрибути автоматично перебувають у повній функціональній залежності від первинного ключа.

--Друга нормальна форма застосовується лише до таблиць, які у першій нормальній формі. Після застосування другої форми, 
...
--якщо атрибут залежить від атрибуту В, а атрибут залежить від атрибуту А (за умови, що атрибут А функціонально не залежить ні від атрибуту В, 
--ні від атрибуту С).
...
--Для нормалізації вихідної таблиці необхідно атрибути, які у транзитивній залежності від ключа, 
--винести в окрему таблицю з копією того атрибута, від якого вони безпосередньо залежать.
...
--
SELECT COUNT( Manufacturer)  --DISTINCT
FROM Products
...
--
SELECT SUM(ProductCount * Price) 
FROM Products

--
SELECT AVG(DISTINCT ProductCount) 
AS Average_Count
FROM Products

--
SELECT AVG(ALL ProductCount) 
AS Average_Count
FROM Products
...
SELECT Manufacturer, ProductName, MIN(Price) AS MinPrice
FROM Products
GROUP BY Manufacturer, ProductName
HAVING MIN(Price) = (Select MIN(Price) FROM Products)

SELECT Manufacturer, ProductName, MAX(Price) AS MaxPrice
FROM Products
GROUP BY Manufacturer, ProductName
...
SELECT Manufacturer, COUNT(*) AS ModelsCount, ProductName, Price * ProductCount AS TotalPrice
FROM Products
WHERE Price * ProductCount > 100000
GROUP BY Manufacturer, ProductName, Price * ProductCount
HAVING COUNT(*) > 1
...
--Але існують корелюючі підзапити (correlated subquery), результати яких залежить від рядків, які вибираються в іншому запиті.
...
--Корелюючий підзапит може виконуватися і для тієї ж таблиці, до якої виконується основний запит. 
...
--Слід враховувати, що корелюючі підзапити виконуються для кожного окремого рядка вибірки, 
...
--
SELECT *,
      (SELECT  AVG(Price) FROM Products) AS AvrgPrice
FROM Products
WHERE Price > (SELECT AVG(Price) FROM Products)
...
SELECT Customers.FirstName, Orders.CreatedAt, Orders.ProductCount, Products.ProductName 
FROM Orders
JOIN Products ON Products.Id = Orders.ProductId
...
WHERE  Products.Id = Orders.ProductId AND Products.Manufacturer='Apple'
...
--
SELECT FirstName, LastName, AccountSum + AccountSum * 0.1 AS TotalSum , 0.1 AS Cons
FROM Customers WHERE AccountSum < 3000
UNION SELECT FirstName, LastName, AccountSum + AccountSum * 0.3 AS TotalSum , 0.3 AS Cons
FROM Customers WHERE AccountSum >= 3000
...
SELECT * FROM Orders
WHERE DATEDIFF(day, CreatedAt, GETDATE()) = 2263
-------------------------------
-- Перетворення даних
...
--
SELECT Id, CAST(CreatedAt AS nvarchar) + '; total: ' + CAST(Price * ProductCount AS nvarchar) AS 'Order'
FROM Orders
